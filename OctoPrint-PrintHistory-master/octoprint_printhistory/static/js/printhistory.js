{$(function() {
    function PrintHistoryViewModel(parameters) {
        ...
        self.repeatPrint= function(data)
        {

            console.log(data["filename"]);
            var index=self.findFileData(data["fileName"]);
            console.log(self.fileData["files"][index]);
            if(index !="none") {

                //console.log(data.fileName);
                $.ajax(
                    {
                        url: "/api/files/"+self.fileData["files"][index]["origin"]+"/" + data.fileName,
                        type: "POST",
                        dataType: "json",
        ...
        self.findFileData=function(filename)
        {
            ...
            {
                if(self.fileData["files"][index]["name"]==filename)
                {
        ...
        self.displayData=function(data)
        {


        }
        self.doesExist=function(data)
        {
            ...
            for(var index=0; index<self.fileData["files"].length;index++)
            {
                if(self.fileData["files"][index]["name"]==data["fileName"])
                {
                    return true;
                }
        ...
        self.requestData = function() {
            ...
            self.getFilesInfo().done(function(result)
            {
